...
float _UseProbe, _ShadowToggle;
samplerCUBE _Cube;
float4 _Cube_HDR;
fixed4 _FakeLight, _ShadowColor;
half3 worldPos;
half atten;
...
float3 Lightprobe_vec()
{
	float3 Lightprobe_vec = (unity_SHAr.xyz*unity_SHAr.w + unity_SHAg.xyz*unity_SHAg.w + unity_SHAb.xyz*unity_SHAb.w);
	UNITY_FLATTEN
	if ( length( Lightprobe_vec) > 0)
	{Lightprobe_vec = Unity_SafeNormalize(Lightprobe_vec);}
	else 
	{Lightprobe_vec = float3(0,0,0);}
	return Lightprobe_vec;
}
